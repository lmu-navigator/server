...
public class RoomServlet extends HttpServlet {
	...
	final String[] csvParams = new String[] { "GCode", "Stadtcode",
			"Straßencode", "BWCode", "BTCode", "Geschoss", "Benennung",
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (fileItem.isFormField()) {
					...
					/* import lines (1;n) */
					List<Room> result = read(fileItem.getInputStream());
					...
					if (rowsSaved == result.size()) {
						settings.addVersion("uploaded CSV for 6_room");
	 ...
	 * @return
	 */
	private List<Room> read(InputStream input) {
	...
	private int saveToDB(List<Room> input) {
		...
		try {
			...
			for (Room c : input) {
				int n = db.writeRoom(c.getCode(), c.getName(),
						c.getFloorCode(), c.getPosX(), c.getPosY());
				if (n > 0)	i++;
			}
