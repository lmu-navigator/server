...
public class BuildingPartServlet extends HttpServlet {
	...
	final String[] csvParams = new String[] {"BTCode", "Stadtcode", "Straßencode", "BWCode", "BauteilHaus"};
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (fileItem.isFormField()) {
					...
					/* import lines (1;n) */
					List<BuildingPart> result = read(fileItem.getInputStream());
					...
					if (rowsSaved == result.size()) {
						settings.addVersion("uploaded CSV for 4_building_part");
	 ...
	 * @return
	 */
	private List<BuildingPart> read(InputStream input) {
		List<BuildingPart> list = new ArrayList<BuildingPart>();
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				BuildingPart e = new BuildingPart();
	...
	private int saveToDB(List<BuildingPart> input) {
		
		BuildingPartMySQL db;
		int i = 0;
		
		try {
			db = new BuildingPartMySQL();
			
			for (BuildingPart c : input) {
				int n = db.writeBuildingPart(c.getCode(), c.getBuildingCode(), c.getAddress());
				if (n > 0)	i++;
			}
