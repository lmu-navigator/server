...
public class SettingsMySQL {
	...
	/* ------------------- HTTP LOCATION ------------------- */
	public String readHttpLocation() {

		try {
			preparedStatement = null;
			preparedStatement = connect
					.prepareStatement("SELECT * FROM settings WHERE parameter = 'httpLocation'");
			resultSet = preparedStatement.executeQuery();
	...
	public boolean updateHttpLocation(String httpLocation) {

		try {
			// update Street
			preparedStatement = null;
			preparedStatement = connect
					.prepareStatement("UPDATE `settings` SET `value` = ? WHERE `settings`.`parameter` = 'httpLocation'");

			preparedStatement.setString(1, httpLocation);
			
			if (preparedStatement.executeUpdate() > 0) {
				System.out.println("updated httpLocation in DB to: "+httpLocation);
				return true;
			} else { 
				addHttpLocation(httpLocation);
			}
	...
	public boolean addHttpLocation(String httpLocation) {
		try {
			preparedStatement = null;
			preparedStatement = connect
					.prepareStatement("INSERT INTO `settings` VALUES (?, ?)");
			...
			preparedStatement.setString(1, "httpLocation");
			preparedStatement.setString(2, httpLocation);
			
			if (preparedStatement.executeUpdate() > 0) {
				System.out.println("updated httpLocation in DB to: "+httpLocation);
				return true;
			} else {
				System.out.println("Error adding httpLocation");
				return false;
			}
		} catch (Exception e) {
			e.printStackTrace();
